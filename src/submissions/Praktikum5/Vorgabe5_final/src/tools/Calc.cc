...
/**
     ...
     * eine Zeile auf dem screen-Bildschirm haben (siehe
     * EXPR_SIZE_MAX).
     */
 ...
 { 
    ...
    else if (location < EXPR_SIZE_MAX)   //Expr Size                                          <----           -----BADR NB
    {
         ...
         if (interp.isWhitespace(c))
        {
            ...
            // here we will be able to add the rest of the charachters to the cga, where we add them to the buffer
            helpInInsert();
            buffer[location] = c;
            location++;
            out.print(c);
...
void Calculator::moveLeft()
{
    ...
    // otherwise we will leave screen screen
    if (row == 0 && column == 0)
    {
        location = 0;
