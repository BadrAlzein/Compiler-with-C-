 ...
 * Edit R:
 * These, wie es geht:
 *      Hier buffern wir den ScanCode, der reinkommt im Buffer 'scanCodeBuffer'.
 *      Danach lesen wir diesen im Epilog wieder aus.
 *
 *      Warum hier buffern und nicht im Epilog?
 *          ==> Wir müssen den ScanCode unbedingt abspeichern. Da wir Schreiber werden, bewegen wir uns in einem
 *              kritischen Abschnitt. Die Interrupts sollen also nicht an sein.
 *          ==> Vor 'pick.ack()' (Interrupts wieder aktivieren), den ScanCode abspeichern.
 *
 * @return
...
bool Keyboard::prologue() {
    ...
    if (ctrlPort.read() & AUX_BIT) {
        ...
        counter++;
        unsigned char code = dataPort.read();
        scanCodeBuffer.add(code);
    }
    // Auch möglich mit 'pic.ack();'
    pic.ack(PIC::KEYBOARD);
    return true; // siehe Clock.cc
 ...
 * Edit R:
 *
 * Die Interrupts sind hier angeschaltet.
 *
 */
